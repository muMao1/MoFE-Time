...
from time_moe.utils.log_util import get_logger
from models.configuration_mofe import MoFEConfig
from models.loss_function import CauchyMseLoss
...
class MoFEInputEmbedding(nn.Module):
    """
    Use a mlp layer to embedding the time-series.
    """

    def __init__(self, config: MoFEConfig):
        super().__init__()
        self.config = config
...
class MoFERotaryEmbedding(torch.nn.Module):
...
class MoFERMSNorm(torch.nn.Module):
...
class MoFEFANBlock(nn.Module):
    """
        MoFEFANBlock: The layer used in FAN (https://arxiv.org/abs/2410.02675).
    ...
    def __init__(self, hidden_size, output_dim, hidden_act='gelu', p_ratio=0.06):
        super(MoFEFANBlock, self).__init__()
        use_p_bias = False
    ...
    def forward(self, src):
        ...
        #p_gate = self.activation(self.input_linear_p_gate(src))
...
class MoFEExpertsLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.model_type == 'mofe_fan':
            get_logger("log").info('========= Use FAN in ExpertsLayer =========')
            ...
            self.shared_expert = MoFEFANBlock(
                hidden_size=self.config.hidden_size,
                output_dim=self.config.hidden_size,
                hidden_act=self.config.hidden_act,
                p_ratio=self.config.p_ratio
            )
        elif self.model_type == 'mofe_ffn':
            get_logger("log").info('========= Use FFN in ExpertsLayer =========')
            self.experts = nn.ModuleList(
                [MoFEFFNBlock(
                    hidden_size=self.config.hidden_size,
            ...
            self.shared_expert = MoFEFFNBlock(
                hidden_size=self.config.hidden_size,
                intermediate_size=self.config.intermediate_size,
                hidden_act=self.config.hidden_act,
            )
        else:
            raise ValueError("model_type must be one of following choices: [mofe_fan, mofe_ffn]")
...
class MoFEAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: MoFEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = MoFERotaryEmbedding(
            self.head_dim,
...
class MoFEDecoderLayer(nn.Module):
    def __init__(self, config: MoFEConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size

        self.self_attn = MoFEAttention(config, layer_idx)

        self.ffn_layer = MoFEExpertsLayer(config)
        self.input_layernorm = MoFERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MoFERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MoFEPreTrainedModel(PreTrainedModel):
    config_class = MoFEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["MoFEDecoderLayer"]
...
class MoFEModel(MoFEPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MoFEDecoderLayer`]
    ...
    def __init__(self, config: MoFEConfig):
        ...
        self.embed_layer = MoFEInputEmbedding(config)
        self.layers = nn.ModuleList(
            [MoFEDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = MoFERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MoFEOutputLayer(nn.Module):
...
class MoFEPrediction(MoFEPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: MoFEConfig):
        ...
        # self.loss_function = torch.nn.HuberLoss(reduction='none', delta=0.2)
        self.loss_function = CauchyMseLoss(delta=2.0)
    ...
    def calc_ar_loss(self, predictions, labels, loss_masks, horizon_length):
        ...
        # mapping loss on sequence level
